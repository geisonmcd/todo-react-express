...
router.post("/updateTask", (req, res) => {
  ...
  connection.query(
    "UPDATE tasks SET task_name = ?, task_date = ? WHERE id=?",
    [taskName, taskDate, targetId],
...
router.post("/completeTask", (req, res) => {
  ...
  connection.query(
    "UPDATE tasks SET finished = NOT finished WHERE id=?",
    [targetId],
    (error, results, fields) => {
      if (error) {
        res.json(error);
...
router.post("/deleteTask", (req, res) => {
  connection.query(
    "DELETE FROM tasks WHERE id = " + req.body.taskId,
    (error, results) => {
      if (error) {
        res.json(error);
      } else {
...
router.post("/addTask", (req, res) => {
  ...
  connection.query(
    "INSERT INTO tasks (task_name, task_date) VALUES (?, ?)",
    [newTask, newTaskDate],
    (error, results) => {
      if (error) {
        res.json(error);
